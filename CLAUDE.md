...
- `Components/Pages/*.razor` — the Blazor UI (Dashboard, Learn, ToeicCenter, SpeakingPractice, WritingPractice, ListeningLab, Onboarding, Placement, Privacy, Terms, Admin pages, etc.) — interactive-server render mode, no WASM. Learner-facing copy is Thai; the English in lessons is the subject matter, not the interface language.
...
- `Infrastructure/` — cross-cutting concerns: `StartupSecurityValidator` (fails fast on unsafe production config, see below), `SecurityHeadersMiddleware` + `CspNonce`, `GlobalExceptionHandler`, `ThaiIdentityErrorDescriber` (Thai validation messages from Identity itself), `DistributedRateLimiting` (Redis-backed, only when `MultiInstance:Enabled`), health checks, `LearningTelemetry` (OpenTelemetry ActivitySource/Meter)

**Data/domain model**: entities live in `EnglishMasterAI.Data/Domain/LearningModels.cs` and are exposed via `ApplicationDbContext` (`LearnerProfile`, `CourseModule`, `Lesson`, `VocabularyItem`, `AssessmentQuestion`, `LearningProgress`, `PlacementAttempt`, `ReviewSchedule`, `WritingSubmission`, `SpeakingSubmission`, `ContentRevision`, `AuditFinding`, `LearningActivity`, `LearnerAchievement`, `AiUsageRecord`, `ContentReviewAssignment`). Seeding logic is in `EnglishMasterAI.Web/Data/SeedData.cs`, which owns the database and idempotency concerns only — the curriculum and assessment material it seeds lives in `content/curriculum/*.json` and is read by `SeedContent.cs`. That split is deliberate and licence-driven: the code is Apache 2.0 while the course content is reserved, so content must not be inlined back into source (see `NOTICE`). `toeic-mock-seeds.json` holds authored source material rather than finished questions; `SeedContent.BuildToeicMockQuestions` expands it into exactly 200 questions, rotating the correct answer position.
...
**Security posture baked into startup**: `StartupSecurityValidator` runs synchronously in `Program.cs` before the app starts serving and throws on unsafe Production configuration. It rejects `SeedAdmin`, wildcard `AllowedHosts`, SQLite, and unconfirmed-email flows unless explicitly overridden, and additionally *requires* `Proxy:Enabled` with at least one `KnownProxies`/`KnownNetworks` entry, plus a fully populated `Legal` section. Read the class rather than re-deriving the rules from scattered options. Note the checks fire only when the environment is Production — both compose files run as `Staging`, so they do not exercise them.

**CSP forbids inline script**: `SecurityHeadersMiddleware` issues a per-request nonce (`CspNonce`) and `script-src` is `'self' 'nonce-…'` with no `'unsafe-inline'`. Inline `onclick=` attributes and bare `<script>` blocks are blocked at runtime — this is why the nav toggle is a component-bound button and why fonts are vendored locally instead of pulled from a CDN. Anything new that needs inline script has to carry the nonce, as `App.razor` does for the Blazor import map.

**Legal consent**: registration requires accepting the terms and privacy notice, and stores both the timestamp and the accepted document version (`LegalDocumentVersions`) on `ApplicationUser`, so consent evidence survives revisions of either document.
...
**Content and licensing**: the code is Apache 2.0 (`LICENSE`) while the course material is all rights reserved, and `NOTICE` draws the line. Everything reserved sits under `content/` or `roadmap/` — seeded curriculum in `content/curriculum/*.json`, the TOEIC media manifest in `content/toeic-media/manifest.json` (validated by `scripts/Test-ToeicMediaManifest.ps1` in CI), syllabus source docs in `roadmap/*.txt`. Keep it that way: lesson text, question banks, and answer keys must not be inlined back into `.cs` or `.razor`, because the boundary is meant to be readable from the file layout rather than from a paragraph in `NOTICE`.
